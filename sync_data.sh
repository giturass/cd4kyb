...
backups = [file for file in client.list() if file.endswith('.tar.gz') and file.startswith('aicd_backup_')]
if not backups:
    print('没有找到备份文件')
    sys.exit()
...
sync_data() {
    while true; do
        ...
        if [ -d "/app/data" ]; then
            ...
            backup_file="aicd_backup_${timestamp}.tar.gz"
...
" 2>&1
            ...
            echo "data directory does not exist, waiting for next sync..."
        fi
